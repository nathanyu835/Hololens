...
/// </summary>
public class ProspectControl : NetworkBehaviour
{
    // Can't easily make network behaviors singletons with the template
    private static ProspectControl _Instance;
    public static ProspectControl Instance
    {
        get
        {
            ProspectControl[] objects = FindObjectsOfType<ProspectControl>();
            if (objects.Length != 1)
            {
                Debug.LogFormat("Expected exactly 1 {0} but found {1}", typeof(ProspectControl).ToString(), objects.Length);
            }
            else
            {
                _Instance = objects[0];
...
//    SyncListBool AtGoal = new SyncListBool();
    ...
    void Awake()
    {
        ...
        var allChildren = world.GetComponentsInChildren<Transform>();
        ...
        Debug.Log("attempted to add colliders");
    }
    ...
    void Update()
    {
        ...
        if (Immersed)
        {
            ...
            Quaternion rotToSend = Quaternion.Inverse(transform.rotation);
            rotToSend *= playerController.transform.localRotation;

            Vector3 modelLocalPosition = transform.InverseTransformPoint(playerController.transform.position);
    ...
    void DrawRemoteGaze()
    {
        foreach (KeyValuePair<string, LevelPlayerStateData> players in systemIdToPlayerState)
        {
            if (players.Value.Immersed == false && players.Value.GazeIndicator != null)
            {
                ...
                {
                    Debug.Log("No immersed avatar...");  // HERE IS THE PROBLEM
                }
#if LINE_REND
    ...
    public void OnPathMessage(int PathIndex, string PlayerName)
    {
        ...
        else
        {
            ...
            if (lpsd.Immersed)
            {
                ...
                lpsd.ImmersedAvatar.transform.SetParent(transform, false);
                ConfigureAvatarsForPathState();
            }
